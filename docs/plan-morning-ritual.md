...
Slot 0 is the gym, already being planned in parallel. The order below is for everything after it. Each entry is a future idea brief, not a commitment. **This list was rewritten April 9** after narrowing the scope around "writing practice, not life tracker" (see §9).

1. **Slot 0, Gym.** Already in flight. Slot zero because it is the specific pain that triggered the ritual and because the interview pressure is real and current.
2. **Slot 1, Inner weather.** The emotional two-sides slot — what's weighing, what's avoided, what needs to be said / what's giving life, gratitude. Free-form writing under each header, not a form. Template lives in `inner-weather/template.md` and is editable by hand; Day-1 draft is saved as `.llm/inner-weather-template.md`. First after the gym because it is the highest-value capture Brandon already proved over months in Capacities, and it needs zero integrations.
3. **Slot 2, Piano (workspace).** The richest slot in the initial set, because piano is where Brandon most wants ongoing support. Structure: `piano/state.md` is a persistent file — *"currently working on Chopin Op. 9 No. 1 LH voicing; next up: phrase shaping bars 12–16"* — that Brandon edits whenever his practice focus shifts. Morning run prints `state.md` as a standing reminder of what he's working on and asks one optional prompt ("anything from yesterday's practice?", skippable). Adhoc run (`ritual piano`, typically after a 4pm practice session) opens a richer prompter: what did you work on, what clicked, what frustrated, any changes to `state.md`? The state file is the living workspace; the day log captures time-stamped session reflections. This is the slot that most clearly earns the "bullet journal collection" framing (§1) and the slot that first demonstrates the `RITUAL_MODE` pattern (§10). Built second because piano is where Brandon most wants support *beyond* the morning window, and proving the adhoc entry point here sets the pattern for every later workspace slot.
4. **Slot 3, Practices reminder.** Minimum-viable reminder slot. A single file (`practices/reminder.md`) lists Brandon's standing practices — movement, meditation, water, whatever he wants to acknowledge. The `run.sh` just prints the file and exits. No prompt, no capture. Brandon edits `reminder.md` when his practices shift (monthly, maybe). The point is not to track; the point is to acknowledge the standing shape of his days and reinforce that they are still happening. **Upgrade path** (explicit, for when any specific practice earns it): a single practice inside `reminder.md` can graduate to its own richer slot. A meditation retreat might prompt a `meditation/` slot that captures reflections; a new movement routine might prompt a `movement/` slot. Graduation is: create the new slot dir, move the relevant block from `reminder.md` into the new slot's state file, write a richer `run.sh`. No framework, no plugin API — just filesystem moves. Practices slot built third because it is trivially small, proves the "reminder slot" pattern (§10), and honors meditation/movement without forcing daily writing Brandon does not need to do.
5. **Slot 4, Dreams.** A single slot for dream capture. `$EDITOR` opens with one header (`## Dreams`) and whatever wants to come out. Built fourth because dreams are their own thing (distinct from emotional work and distinct from meditation), because capture is friction-sensitive right after waking, and because skipping is cheap on dreamless mornings.
6. **Slot 5, Reading reflections.** Built before creative-project check-in because it has the clearest handoff to crumb.blog (the `narrow-agent-telegram-breadcrumbs` pattern is already proven) and because it produces public artifacts that reinforce the habit. Likely the first slot to activate the sidecar bus (§10) — a later "crumb.blog draft" slot reads its output. May also have a small state file (`reading/currently.md` — what Brandon is reading right now) following the piano pattern.
7. **Slot 6, Creative project check-in.** What's surfacing, what's maturing, what wants attention this week. Two or three free-form headers, no status-tracking, no percentages. Probably also a workspace slot with a `state.md` for ongoing projects. Built after reading because reading often feeds creative work (a book catalyzes an idea), so the natural data flow is reading → creative.
8. **Slot 7, Creative writing prompt.** First pure generated slot. Pulls a prompt from a rotating source — a plain text file Brandon maintains, an API, or an LLM with a fixed seed — and opens `$EDITOR` for a response. The source is a file Brandon edits directly; swapping it is one line. A gentle test of the fetch-slot pattern before the integration-heavy ones.
9. **Slot 8, Daily briefing.** One slot combining quote + calendar peek + optional elder-care digest. Read-only. Confirmed April 9 as one bundled slot rather than three — they all fail the same way (no network → skip with a note) and bundling keeps the morning flow tight. Can still split later if any one of them grows teeth. Late in the phasing because each integration is real work (auth, API keys, graceful offline degradation) and the ritual must work without network.
10. **Slot 9, Tend (read-only surface with link).** Day-1-able minimum: prints a link that opens Tend in the browser, plus a note that this slot will later grow into a full read-only API integration. On the first build, it's a 3-line slot that says *"Today in Tend: https://tendyourgarden.app/today"* and exits. Later upgrade reads today's tasks and the frog via Tend's API. Confirmed April 9 that the link-first shape is the right starting point: it honors the "ritual must work offline" rule, gives Brandon instant value, and leaves the API integration as an obvious upgrade path that does not break anything. Explicitly read-only — project planning stays in Tend.

**Dropped from the v1 phasing (April 9):** the brain dump slot, the compass slot (values check + frog + day plan), the habit-experiment tracker (carbs, sleep). These were scaffolding for a life-tracker framing Brandon has rejected. The values named in the original Capacities template (curiosity, compassion, courage, consistency) stay as personal context in `NOTES.md`, not as a daily check-in.

Rule of thumb, unchanged from v1: no slot gets planned until the previous slot has been used in real mornings for at least a week without being abandoned.
